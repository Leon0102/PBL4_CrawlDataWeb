...
public class Hose_DAO {
public static ObservableList<Hose> findAll(){
		
		Connection conn = MySQLConnection.connectDb();
		ObservableList<Hose> data = FXCollections.observableArrayList();
		try {
			String query = "SELECT * FROM hose";
	...
	public void insert(Hose ck) {
		try {
			Connection con = MySQLConnection.connectDb();
			String query = "Insert Into hose(id,tc,tran,san,mua_gia3,mua_kl3,mua_gia2,mua_kl2,"
...
public static ObservableList<Hose> findTop(){
		
		Connection conn = MySQLConnection.connectDb();
		ObservableList<Hose> data = FXCollections.observableArrayList();
		try {
			String query = "SELECT * FROM `hose` ORDER BY tc DESC LIMIT 30";
			PreparedStatement stm = conn .prepareStatement(query);
			ResultSet rs = stm.executeQuery();
			while(rs.next()) {
...
public void update(Hose ck) {
	try {
		Connection con = MySQLConnection.connectDb();
		String query = "UPDATE `hose` SET `id`='[value-1]',`tc`='[value-2]',`tran`='[value-3]',`san`='[value-4]',`mua_gia3`='[value-5]',"
