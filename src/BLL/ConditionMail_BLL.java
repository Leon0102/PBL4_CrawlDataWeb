...
public class ConditionMail_BLL {
    ...
    public static ConditionMail GetConditionMail(String idStock) throws SQLException
    {
    ...
    public static String NotifyStock(String idStock, String exchangeName) throws SQLException
    {
        String rs="";
        ...
        if(selected.Amount==1) {
            if(c.AmountStart< n.order_Amount&& n.order_Amount<c.AmountEnd )
                rs+="\n-Khoi luong co phieu: "+n.order_Amount;
        }
        if(selected.DTNN==1) {
            if(c.DTNNStart< n.total_buy&& n.total_buy<c.DTNNEnd )
                rs+="\n-Khoi luong dau tu nuoc ngoai: "+n.total_buy;
        }
        if (rs != "")
        {
        	rs = "\n" +exchangeName +" - "+idStock + rs;
        }
        return rs;
    }
    public static ObservableList<ConditionMail> GetAllCondition()
    {
        return ConditionMail_DAO.GetAllCondition();
    }
}